"""Browser regression checks for the staged portfolio and three-project slideshow."""
import functools
import http.server
import json
import os
...
try:
    with sync_playwright() as p:
        ...
        page.locator('[data-show-slide="0"]').focus()
        page.keyboard.press('ArrowRight')
        check('Arrow key navigation',page.evaluate('portfolioState().slideshow.index') == 0)
        page.mouse.move(0,0)
        check('Focus stops autoplay until requested',not page.evaluate('portfolioState().slideshow.playing'))
        page.locator('#slideshow-toggle').focus()
        page.keyboard.press('Enter')
        page.wait_for_timeout(200)
        check('Explicit play resumes autoplay',page.evaluate('portfolioState().slideshow.playing'))
        ...
        page.click('#slide-prev')
        check('Manual navigation works while paused',page.evaluate('portfolioState().slideshow.index') == 2)
        ...
        # Touch gesture changes the project without opening a gallery.
        page.locator('#hero-showcase').scroll_into_view_if_needed()
